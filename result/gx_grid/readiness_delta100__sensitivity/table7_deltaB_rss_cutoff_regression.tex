\begin{table}[H]\centering
\begin{threeparttable}
\caption{Full-Theta RSS Cutoff and Debt-Change Dynamics}
\label{tab:deltaB_rss_cutoff_fulltheta}
\scriptsize
\begin{tabularx}{\textwidth}{lY}
\toprule
Dependent variable & $$B_{i,t+1}-B_{it}$$ \\
\midrule
$$G_{it}\times 1\{\widehat{\theta}^{F}_{it}<\widehat{c}\}$$ & -0.046 \\
 & (-0.892) \\
$$G_{it}\times 1\{\widehat{\theta}^{F}_{it}\geq\widehat{c}\}$$ & -0.250*** \\
 & (-3.555) \\
Real GDP & 7.782*** \\
 & (5.195) \\
Real GDP growth & -0.347*** \\
 & (-3.815) \\
CPI inflation & -0.180** \\
 & (-2.089) \\
Overall balance/GDP & -0.482*** \\
 & (-5.851) \\
International reserves & 0.020 \\
 & (1.403) \\
Government effectiveness & -1.862** \\
 & (-1.989) \\
Regulatory quality & 1.181 \\
 & (1.350) \\
Terms of trade & 0.041*** \\
 & (2.688) \\
$$\lambda_L-\lambda_H$$ & 0.203*** \\
 & (3.011) \\
\midrule
RSS-selected cutoff $$\widehat{c}$$ & 8.033 \\
Empirical percentile of $$\widehat{c}$$ & 77.45 \\
RSS at $$\widehat{c}$$ & 1.954 \\
Low-theta observations & 821 \\
High-theta observations & 239 \\
Controls & Yes \\
Country FE & Yes \\
Year FE & Yes \\
$$p_L$$ at RSS cutoff & 0.373 \\
$$p_H$$ at RSS cutoff & 0.000 \\
$$p$$-value: $$\lambda_L=\lambda_H$$ & 0.003 \\
Observations & 1,060 \\
Countries & 60 \\
Adjusted $$R^2$$ & 0.434 \\
\bottomrule
\end{tabularx}
\begin{tablenotes}[flushleft]\footnotesize
\item Notes: The RSS cutoff is selected by minimizing RSS over empirical values of $$\widehat{\theta}^{F}_{it}$$ that leave nonempty low- and high-theta groups. For each candidate cutoff and for the final regression, the model includes regime-specific readiness slopes, the common control vector $$Z_{it}$$, country fixed effects, and year fixed effects. Robust t-statistics are reported in parentheses. *** $$p<0.01$$, ** $$p<0.05$$, * $$p<0.10$$.
\end{tablenotes}
\end{threeparttable}
\end{table}
